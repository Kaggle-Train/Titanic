...
class MotherChildProcessor:
    ...
    def transform_train(self, X):
        ...
        for index, row in X.iterrows():
            if (row.Age < self.child_age_margin):
    ...
    def transform_test(self, X):
        ...
        for index, row in X.iterrows():
            for index2, row2 in self.X_eval.iterrows():
                if (row.LastName == row2.LastName):
                    if (self.is_row_mother(row) or (row.Age < self.child_age_margin and self.is_row_mother(row2))):
                        if (row2.Survived == 1):
                            X.loc[index, 'MotherChildRelation'] = 1
                        else:
                            X.loc[index, 'MotherChildRelation'] = -1
    ...
    def is_row_mother(self, row):
        if (row.Sex == 'female' and row.Age > self.mother_age_margin):
            return True
        return False
